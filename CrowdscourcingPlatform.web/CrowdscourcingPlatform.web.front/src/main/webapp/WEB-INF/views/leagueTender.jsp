...
<!----------------------------------------------- 头部 ------------------------------------------------->
		<div class="div-logArea">
			...
			<div class="layui-form my-form" action="">
				...
				<div class="layui-form-item">
					<div class="layui-input-block">
						<button class="layui-btn" id="tender" lay-submit lay-filter="sent" needId=${needIdBudding} leaId=${leaMegBuding.id}>立即提交</button>
						<input type="button" class="layui-btn layui-btn-primary" id="close" onclick="retNeed()" value="返回">
					</div>
				</div>
			</div>
		</div>
...
<!---------------------------------------------------------------------------底部-------------------------------------------------------------->
				...
				layui.use('form', function() {
					...
					form.on('submit(sent)', function(data) {
						...
						if(accept==false){
						  ...
						  $.ajax({
							url: "buddingTenderLeague.action",
							type: "post",
							data: {
								"needId": needId,
								"content":content,
		...
		function retNeed(){
			...
			function refush(){
				location.href="leagueCenter.html";
			}
			//动态生成友情链接
			function initLinker() {
				$.ajax({
