...
<body>
	...
	<script>
		...
		function showAccounts() {
			...
			$.ajax({
				url : "do-showProviderAccounts.action",
				type : "post",
				data : {
					"start" : start,
					"limit" : limit,
				...
				success : function(msg) {
					...
					for (var i = 0; i < accounts.length; i++) {
						...
						var state="";
						var money="";
						if(acc.state==1&&acc.serName==acc.useName){
							state="交易完成";
							money="+"+(-acc.money);
						}
						if(acc.state==1&&acc.serName!=acc.useName){
						...
						if(acc.state==2&&acc.serName==acc.useName){
							state="收款";
							money="+"+(-acc.money);
						}
						if(acc.state==2&&acc.serName!=acc.useName){
						...
						if(acc.state==3&&acc.serName!=acc.useName){
							state="收到退款";
							money="+"+acc.money;
						}
						if(acc.state==3&&acc.serName==acc.useName){
